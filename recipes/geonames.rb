...
deploy "#{node['pelias']['basedir']}/geonames" do
  ...
  only_if  { node['pelias']['geonames']['index_data'] }
end
...
node['pelias']['geonames']['alpha2_country_codes'].each do |country|
  ...
  log "log geonames load for #{country}" do
    action  :nothing
    message "Beginning load of Geonames data into Elasticsearch for #{country}. " \
            "Log: #{node['pelias']['basedir']}/logs/geonames_#{country}.{out,err}"
  end

  execute "load geonames for #{country}" do
    action  :nothing
    user    node['pelias']['user']['name']
    command "./bin/pelias-geonames -i #{country} " \
