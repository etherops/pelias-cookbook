...
deploy "#{node['pelias']['basedir']}/osm" do
  ...
  only_if  { node['pelias']['osm']['index_data'] }
end
...
node['pelias']['osm']['extracts'].map do |name, url|
  ...
  log "log osm load #{name}" do
    action  :nothing
    message "Beginning load of OSM data into Elasticsearch for #{name}. " \
            "Log: #{node['pelias']['basedir']}/logs/osm_#{name}.{out,err}"
  end
